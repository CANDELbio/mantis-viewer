...
export class ImageSetSelector extends React.Component<ImageSetSelectorProps, {}> {
    ...
    private onPersistDataChange = (event: React.FormEvent<HTMLInputElement>) =>
        this.props.setPersistData(event.currentTarget.checked)
        ...
        if (this.props.imageSetOptions.length > 0) {
            return (
                <div>
                    ...
                    />
                    <Checkbox
                        checked={this.props.persistData}
                        label="Copy Image Set Visualization Settings"
                        onChange={this.onPersistDataChange}
                    />
                </div>
            )
        } else {
