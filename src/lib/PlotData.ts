...
export class PlotData {
    ...
    private static buildSegmentToSelectedRegionMap(
    ...
    private static buildSelectedRegionMap(
        selectedRegion: SelectedPopulation[] | null,
    ...
    private static getSegmentIntensity(
    ...
    private static buildSelectionIdArray(selectedPopulations: SelectedPopulation[] | null): string[] {
    ...
    private static getSelectionName(
        selectionId: string,
    ...
    private static getSelectionColor(
        selectionId: string,
        selectedRegionMap: { [key: string]: SelectedPopulation },
    ): string {
        let color: number
    ...
    private static newPlotDatum(numValues: number): { values: number[][]; text: string[] } {
        let values = []
    ...
    private static calculateRawPlotData(
        channels: string[],
    ...
    private static calculatePlotData(
        channels: string[],
        segmentationData: SegmentationData,
        segmentationStatistics: SegmentationStatistics,
        plotType: PlotType,
        ...
        let rawPlotData = this.calculateRawPlotData(
            channels,
            segmentationData,
            segmentationStatistics,
            plotStatistic,
        ...
        let minMax =
            plotStatistic == 'mean'
                ? segmentationStatistics.meanMinMaxMap[ch]
                : segmentationStatistics.medianMinMaxMap[ch]
    ...
    private static normalizeIntensitiesByMarker(intensities: number[][]): number[][] {
    ...
    private static normalizeIntensitiesByPopulation(intensities: number[][]): number[][] {
        return intensities.map((population: number[]) => {
    ...
    private static normalizeHeatmapIntensities(
        intensities: number[][],
    ...
    private static calculateHeatmapData(
        segmentationData: SegmentationData,
        segmentationStatistics: SegmentationStatistics,
        plotStatistic: PlotStatistic,
        plotTransform: PlotTransform,
        ...
        for (let selectionId of selectionIds) {
            ...
            for (let channel of channels) {
                let intensity = this.getSegmentIntensity(
                    plotStatistic,
                    channel,
                    selectedSegments,
                    plotTransform,
        ...
        heatmapData.push({
            ...
            y: selectionIds.map((selectionId: string) => {
                return this.getSelectionName(selectionId, selectedRegionMap)
            }),
            type: 'heatmap',
        })
    ...
    public constructor(
        ...
        if (plotType == 'histogram' && channels.length == 1) {
            this.data = PlotData.calculatePlotData(
                channels,
                segmentationData,
                segmentationStatistics,
                plotType,
            ...
            this.data = PlotData.calculateHeatmapData(
                segmentationData,
                segmentationStatistics,
                plotStatistic,
                plotTransform,
