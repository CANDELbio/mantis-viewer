...
export class Db {
    ...
    private initialize(): void {
        ...
        db.prepare(
            `CREATE INDEX IF NOT EXISTS feature_index
             ON features (image_set, feature, segment_id, value)`,
        ).run()
        // For keeping track of features generated by Mantis for auto loading.
        db.prepare(
            `CREATE TABLE IF NOT EXISTS features_generated (
                image_set TEXT NOT NULL
            )`,
        ).run()
        db.prepare(
            `CREATE UNIQUE INDEX IF NOT EXISTS features_generated_index
             ON features_generated (image_set)`,
        ).run()
        db.prepare(
        ...
        db.prepare(
            `CREATE INDEX IF NOT EXISTS settings_index
        ...
        db.prepare(
            `CREATE INDEX IF NOT EXISTS selections_index
             ON selections (id, image_set)`,
        ).run()
        db.close()
    }
    ...
    public insertFeaturesGeneratedImageSet(imageSet: string): void {
        const db = this.getConnection()
